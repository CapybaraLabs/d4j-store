 ...
 */
fun <T> withConnection(factory: ConnectionFactory, closure: (Connection) -> Mono<T>): Mono<T> {
 ...
 */
fun <T> withConnectionMany(factory: ConnectionFactory, closure: (Connection) -> Flux<T>): Flux<T> {
 ...
 */
fun Publisher<out Result>.mapToCount(): Mono<Long> {
...
fun Flux<out Result>.consume(): Flux<Int> = flatMap { it.rowsUpdated }

// these are a bunch of convenience methods to achieve that
fun Statement.executeConsuming(): Flux<Int> = execute().toFlux().consume()
fun Statement.executeConsumingSingle(): Mono<Int> = executeConsuming().toMono()


/**
 * Deserialize the data column of multiple rows into a Flux of the requested class using the passed serde
 */
fun <E> Publisher<out Result>.deserializeManyFromData(clazz: Class<E>, serde: PostgresSerde): Flux<E> {
	return this.toFlux()
 ...
 */
fun <E> Publisher<out Result>.deserializeOneFromData(clazz: Class<E>, serde: PostgresSerde): Mono<E> {
 ...
 */
fun <T> Optional<T>.toPossible(): Possible<T> = map { Possible.of(it) }.orElse(Possible.absent())

/**
 * Collapse a nested Possible<Optional<>> into a Possible
 */
fun <T> Possible<Optional<T>>.collapse(): Possible<T> = toOptional().flatMap { it }.map { Possible.of(it) }.orElse(Possible.absent())


/**
 * Sum elements distinctly into a list.
 */
fun <T> sumDistinct(original: Collection<T>, elements: Collection<T>): List<T> {
 ...
 */
fun ReactionData.equalsEmoji(emojiData: EmojiData): Boolean {
